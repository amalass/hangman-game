...
getData().then((resolved) => {
            ...
            if (!element.classList.contains(`clickable`)) {
              ...
              if (example.includes(element.textContent)) {
                ...
                if (
                  ...
                  if (lose !== 0)
                    result.innerHTML = `Congrats!!<br> but you have made ${lose} mistakes :|<button>Retry</button>`;
                  else
                    result.innerHTML = `Congrats!!<br> you have made ${lose} mistakes :)<button>Retry</button>`;
                ...
                if (lose === 11) {
                  result.style.cssText = `visibility: visible;transform: translate(-50%, 0%) scale(1.5, 1.5);`;
                  result.innerHTML = `Oops! you have lost :(<br> the word is: ${example
                    .join(``)
                    .toLowerCase()}<button>Retry</button>`;
                  for (let i = 3; i < 11; i++) {
  ...
  function resetEvery(result) {
    result.children[1].onclick = () => {
      ...
      for (let i = 3; i < 11; i++) {
        [...lines][i].style.cssText = `animation-play-state: paused`;
      }
      [...keys].forEach((element) => {
